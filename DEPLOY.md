...
| `workers/bookforge-api/` | Cloudflare Workers | 데모가 호출하는 API 프록시 | **Anthropic API 키 (여기에만)** |
...
브라우저 ──▶ Vercel (정적 HTML/JS, 키 없음)
   │
   └──POST /api/blueprint──▶ Cloudflare Worker ──x-api-key──▶ Anthropic API
...
npx wrangler secret put ANTHROPIC_API_KEY # 키 입력 — 저장소에 남지 않는다
...
- [ ] `wrangler secret put ANTHROPIC_API_KEY` 완료 (`/api/health` 가 `configured: true`)
- [ ] `web/site.config.ts` 또는 Vercel 환경변수에 Worker URL
- [ ] `ALLOWED_ORIGINS` 를 실제 도메인으로 좁힘
- [ ] Anthropic Console → Limits 에서 월 예산 상한 설정
